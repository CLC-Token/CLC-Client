...
public class ImagePreviewDelActivity extends ImagePreviewBaseActivity implements View.OnClickListener {
    ...
    private void showDeleteDialog() {
        ...
        builder.setTitle(resources.getString(R.string.lzy_tip));
        builder.setMessage(resources.getString(R.string.lzy_are_you_sure_delete_photo));
        builder.setNegativeButton(resources.getString(R.string.lzy_cancel), null);
        builder.setPositiveButton(resources.getString(R.string.lzy_ok), new DialogInterface.OnClickListener() {
            @Override
    ...
    public void onImageSingleTap() {
        if (topBar.getVisibility() == View.VISIBLE) {
            topBar.setAnimation(AnimationUtils.loadAnimation(this, R.anim.lzy_top_out));
            topBar.setVisibility(View.GONE);
            tintManager.setStatusBarTintResource(Color.TRANSPARENT);//通知栏所需颜色
            //给最外层布局加上这个属性表示，Activity全屏显示，且状态栏被隐藏覆盖掉。
//            if (Build.VERSION.SDK_INT >= 16) content.setSystemUiVisibility(View.SYSTEM_UI_FLAG_FULLSCREEN);
        } else {
            topBar.setAnimation(AnimationUtils.loadAnimation(this, R.anim.lzy_top_in));
